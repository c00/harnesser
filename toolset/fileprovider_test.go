...
func TestFileProviderLoad(t *testing.T) {
	t.Run("loads YAML definitions and skips other entries", func(t *testing.T) {
		...
		assert.Equal(t, []types.ToolDefinition{first, second}, provider.GetDefinitions())
		assert.Equal(t, types.Tools{first.Tool}, provider.GetTools())
	...
	t.Run("replaces definitions when reloaded", func(t *testing.T) {
		...
		require.NoError(t, provider.Load())
		assert.Equal(t, []types.ToolDefinition{second}, provider.GetDefinitions())
	})
...
func writeToolDefinition(t *testing.T, path string, definition types.ToolDefinition) {
	t.Helper()
