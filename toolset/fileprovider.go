...
func (p *FileProvider) GetTools() types.Tools {
	tools := types.Tools{}
...
func (p *FileProvider) GetDefinitions() []types.ToolDefinition {
	return p.toolDefs
}

func (p *FileProvider) GetDefinition(name string) (types.ToolDefinition, error) {
	for _, def := range p.toolDefs {
