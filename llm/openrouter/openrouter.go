...
func New(ctx context.Context, cfg types.LlmConfig, apiKey string) *OpenRouter {
	return &OpenRouter{
		ctx:    ctx,
		config: cfg,
...
func (o *OpenRouter) Config() types.LlmConfig {
	return o.config
}

func convertMessages(messages []types.Message) []openrouter.ChatCompletionMessage {
	...
	for i, msg := range messages {
		...
		for _, part := range msg.Content {
			...
			case types.CacheModeShort:
				cc = &openrouter.CacheControl{Type: "ephemeral"}
			case types.CacheModeLong:
				ttl := "1h"
...
func convertTools(tools []types.Tool) []openrouter.Tool {
	if len(tools) == 0 {
		return nil
	}
...
func (o *OpenRouter) completionRequest(ctx context.Context, messages []types.Message, tools []types.Tool) openrouter.ChatCompletionRequest {
...
func (o *OpenRouter) Generate(ctx context.Context, messages []types.Message, tools []types.Tool) (types.Message, error) {
	...
	if err != nil {
		return types.Message{}, fmt.Errorf("chat completion error: %w", err)
	}

	if len(resp.Choices) == 0 {
		return types.Message{}, errors.New("no choices returned from LLM")
	}
	...
	result := types.Message{
		Role:           types.MessageRole(choice.Role),
		Model:          resp.Model,
	...
	if choice.Content.Text != "" {
		result.Content = []types.MessagePart{{Type: "text", Text: choice.Content.Text}}
	} else if len(choice.Content.Multi) > 0 {
		for _, p := range choice.Content.Multi {
			part := types.MessagePart{Type: string(p.Type)}
...
func convertReasoningDetails(details []openrouter.ChatCompletionReasoningDetails) []types.ReasoningDetails {
	result := make([]types.ReasoningDetails, len(details))
	for i, detail := range details {
		result[i] = types.ReasoningDetails{
...
func (o *OpenRouter) GenerateStream(ctx context.Context, messages []types.Message, tools []types.Tool, callback llm.StreamDeltaFunc) (types.Message, error) {
	if callback == nil {
	...
	if err != nil {
		return types.Message{}, fmt.Errorf("chat completion stream error: %w", err)
	}
	defer stream.Close()
	...
	for {
		...
		if recvErr != nil {
			return types.Message{}, fmt.Errorf("chat completion stream error: %w", recvErr)
		}
		...
		if len(chunk.Choices) > 0 {
			...
			if choice.Delta.Role != "" {
				result.Role = types.MessageRole(choice.Delta.Role)
			}
	...
	if !sawChoice {
		return types.Message{}, errors.New("no choices returned from LLM stream")
	}
	if !sawFinish {
		return types.Message{}, errors.New("LLM stream ended before a finish reason was returned")
	}
	if textContent.Len() > 0 {
		result.Content = []types.MessagePart{{Type: types.PartText, Text: textContent.String()}}
	}
